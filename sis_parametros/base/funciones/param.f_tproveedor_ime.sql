...
DECLARE
    ...
    v_id_inst						integer;
    v_id_pers						integer;
    v_id_persona_prov				integer;
    v_id_institucion_prov			integer;
    ...
    --(may)control para no duplicar el registro de persona e institucion
         IF (v_parametros.id_persona is null) THEN
             IF   exists(select 1
                          ...
                          and  p.id_institucion =  v_parametros.id_institucion ) THEN
             ...
             IF   exists(select 1
                          from param.tproveedor  p
                          where p.estado_reg = 'activo'
                          and  UPPER(p.rotulo_comercial) = UPPER (v_parametros.rotulo_comercial) ) THEN
         ...
         ELSE
             ...
             IF   exists (select 1
                 ...
                 raise exception 'Proveedor ya registrado, ya existe la persona %',UPPER(v_parametros.rotulo_comercial);
             END IF;
             IF   exists(select 1
                          from param.tproveedor  p
                          where p.estado_reg = 'activo'
