...
class ACTRol extends ACTbase{
	...
	function listarRolUsuario(){
		...
		$this->objParam->defecto('ordenacion','rol');
		$this->objParam->defecto('dir_ordenacion','asc');
		if($this->objParam->getParametro('id_rol') != '') {
				 $this->objParam->addFiltro("ts.id_rol in(" . $this->objParam->getParametro('id_rol').")");
		 }

		if ($this->objParam->getParametro('tipoReporte')=='excel_grid' || $this->objParam->getParametro('tipoReporte')=='pdf_grid'){
			$this->objReporte=new Reporte($this->objParam, $this);
			$this->res=$this->objReporte->generarReporteListado('MODRol','listarRolUsuario');
		}
		else {
			$this->objFunSeguridad=$this->create('MODRol');
			$this->res=$this->objFunSeguridad->listarRolUsuario($this->objParam);
		}
		//imprime respuesta en formato JSON para enviar lo a la interface (vista)
		$this->res->imprimirRespuesta($this->res->generarJson());
