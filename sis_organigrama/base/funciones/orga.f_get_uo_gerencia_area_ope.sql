CREATE OR REPLACE FUNCTION orga.f_get_uo_gerencia_area_ope (
  par_id_uo integer,
...
BEGIN
  	v_nombre_funcion = 'orga.f_get_uo_gerencia_area_ope';
    ...
    if (par_id_uo is not null) then
        ...
        if (v_nivel in (2,3,4,6) or v_es_gerencia = 'si') then
        	...
        	if (v_id_uo = v_id_uo_hijo) then
            	...
            	return orga.f_get_uo_gerencia_area_ope(v_id_uo, NULL, NULL);
            end if;
        end if;
