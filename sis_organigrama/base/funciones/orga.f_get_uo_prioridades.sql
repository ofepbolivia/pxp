...
BEGIN
	...
	for v_registros in (with items as (select distinct on (uo.id_uo) uo.id_uo,es.haber_basico
    								from orga.tuo uo
                                    left join orga.tcargo i on i.id_uo = uo.id_uo and i.estado_reg in ('activo','inactivo')
                                    left join orga.tescala_salarial es on es.id_escala_salarial = i.id_escala_salarial
                                    where uo.estado_reg in ('activo','inactivo') and i.estado_reg in ('activo','inactivo') and es.estado_reg in ('activo','inactivo')
                        ...
                        where hijos.estado_reg in ('activo','inactivo') and euo.id_uo_padre!= euo.id_uo_hijo and euo.id_uo_padre = p_id_uo and hijos.estado_reg in ('activo','inactivo')
