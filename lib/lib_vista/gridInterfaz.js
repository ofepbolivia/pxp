...
Phx.gridInterfaz=Ext.extend(Phx.baseInterfaz,{
    ...
    //(f.e.a)filtro sin texto
    bottom_filtro_fields : new Array(),
    bottom_filtro_labels : new Array(),
    ...
    initBottomFilter : function () {
        ...
        for (var i = 0; i < this.Atributos.length; i++) {
            ...
            //(f.e.a)filtro sin texto
            if ('bottom_filtro' in ac && ac.bottom_filtro == true) {
                if ('filters' in ac) {
                    if ('pfiltro' in ac.filters) {
                        this.bottom_filtro_fields.push(ac.filters.pfiltro);
                    } else {
                        this.bottom_filtro_fields.push(ac.config.name);
                    }
                } else {
                    this.bottom_filtro_fields.push(ac.config.name);
                }

                this.bottom_filtro_labels.push(ac.config.fieldLabel);

            }
        }

        // Si hay mas de un bottom_filter
        if (this.bottom_filter_fields.length > 0 || this.bottom_filtro_fields.length > 0) {
            ...
            var configFilter = new Ext.ux.form.SearchField ({
                ...
                maxLength: 200,
                qtip: 'Este filtro se aplica sobre las columnas: ' + (this.bottom_filter_labels.concat(this.bottom_filtro_labels)).toString()
            });
    ...
    eventBottomFilter : function () {
        ...
        if (this.bottom_filter_fields.length > 0 || this.bottom_filtro_fields.length > 0) {
            ...
            this.bottom_filter.on('clearfield', function(f, e){
                ...
                //busqueda sin texto
                this.store.baseParams.bottom_filtro_fields = this.bottom_filtro_fields.toString();
                this.store.baseParams.bottom_filtro_value ='';
                this.onButtonAct();
            ...
            this.bottom_filter.on('initsearch', function(f, e){
                ...
                //busqueda sin texto
                this.store.baseParams.bottom_filtro_fields = this.bottom_filtro_fields.toString();
                this.store.baseParams.bottom_filtro_value = this.bottom_filter.getValue();
                this.onButtonAct();
    ...
    constructor: function(config){
        ...
        //busqueda sin texto
        this.bottom_filtro_fields = new Array();
        this.bottom_filtro_labels = new Array();
