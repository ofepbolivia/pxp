...
Phx.baseInterfaz = Ext.extend(Ext.util.Observable, {
     ...
     * Add button in top tollbar
     * @param {integer} index donde se insertara el boton
     * @param {varchar} id  clave para reconocer el boton
     * @param {Object} param Objeto con la configuracion comun para un boton Ext.button
     */

    addButtonIndex: function(index,id, param) {
        var me = this,
            bconf = {
                disabled: true,
                scope: me,
   ...
   getBoton: function(id) {
    ...
    /*Aumentando para las fuciones de Gantt por defecto*/
    ...
    diagramGanttGrilla: function () {
      var rec = {id_proceso_wf:this.sm.getSelected().data.id_proceso_wf};
      Phx.CP.loadWindows('../../../sis_workflow/reportes/gantt/GanttGrilla.php',
          'Diagrama de Gantt',
          {
              width:'90%',
              height:600
     ...
     * Cambia el nombre del FiedLabel
     * @param name: name del objeto config,
     * newValue: nuevo valor recibido
     *
     */
    changeFieldLabel:function(name, newValue){
        /*this.cm.config.map((n, i) => {
            if(n.dataIndex == name){
                this.cm.config[i].header = newValue;
            }
        });*/
        // cambio por motivo de versiones de navegador
     ...
     * @autor breydi vasquez pacheco
     * De una forma rápida validar una fecha. Devolverá true o false dependiendo de si el formato es correcto o no
     * @param f: fecha procesada
     */
    existeFecha: function(f){
    	var fechaf = f.split("/");
    	var day = fechaf[0];
    	var month = fechaf[1];
    	var year = fechaf[2];
 ...
 */
    ...
    defineMenu: function(buttons) {
        ...
        /*Aumentando para incluir el boton */
        if (this.bgantt) {
            cbuttons.push({
                id: 'b-gantt-' + this.idContenedor,
                iconCls: 'bgantt',
                grupo: this.bganttGroups,
                tooltip: 'Generar el diagrama de Gantt',
                text: 'Gantt',
                xtype: 'splitbutton',
                ...
                scope: this,
                menu: [{
                    text: '<span style="font-size:15px;"><i class="fa fa-picture-o" aria-hidden="true"></i> Gantt Imagen</span>',
                    tooltip: '<b>Muestra el reporte gantt en Imagen</b>',
                    //iconCls: 'baddphoto',
                    handler: this.diagramGantt,
                    scope: this
                }, {
                    text: '<span style="font-size:15px;"><i class="fa fa-file-text-o" aria-hidden="true"></i> Gantt Dinámico</span>',
                    tooltip: '<b>Muestra el reporte gantt en formato HTML</b>',
                    //iconCls: 'bgantt',
                    handler: this.diagramGanttDinamico,
                    scope: this
                }, {
                    text: '<span style="font-size:15px;"><i class="fa fa-table" aria-hidden="true"></i> Gantt Grilla</span>',
                    tooltip: '<b>Muestra el reporte gantt en una Grilla del ERP</b>',
                    //iconCls: 'bgantt',
                    handler: this.diagramGanttGrilla,
                    scope: this
                }]
                })
            }
 ...
 */
    ...
    preparaMenu: function(n) {
        ...
        /*Incluyendo el botton del Gantt Ismael Valdivia 22/10/2021*/
        if (tb && this.bgantt) {
            tb.items.get('b-gantt-' + this.idContenedor).enable()
            }
        /***********************************************************/
        return tb
    },
 ...
 */
    liberaMenu: function() {
        ...
        /*Aumentando para Incluir el botton Gantt (Ismael Valdivia 22/10/2021)*/
        if (tb && this.bgantt) {
            tb.items.get('b-gantt-' + this.idContenedor).disable()
            }
        /*********************************************************************/
        return tb
    },
 ...
 */
    ...
    definirRegiones: function() {
        ...
        if (this.west) {
            ...
            this.regiones.push(this.PanelWest);

            this.PanelWest.on('collapse',function(p){
   ...
   // Abre ventana con reporte en pdf
	successExport:function(resp){
        ...
        //fRnk: se añadió el siguiente código para que descargue el reporte PDF
        var downloadLink = document.createElement("a");
        downloadLink.href = "../../../reportes_generados/"+nomRep;
        downloadLink.download = nomRep;
        downloadLink.click();
