...
BEGIN
	...
	***********************************/
	...
	if(p_transaccion='WF_OBS_INS')then
        ...
        begin
            ...
            -- inicio. incremento breydi vasquez 23/03/2020               
            --captura de correos en copia
              FOR v_record IN 
            ...
            update wf.tobs set
              ...
              estado = case when v_parametros.tipo = 'respuesta_observacion' then 'cerrado' else 'abierto' end
	...
	***********************************/
		...
		begin
            -- Incremento breydi vasquez (23/03/2020)
	        -- Modificar la observacion el funcionario que lo creo en el mismo dia de registro.
        
			if ((select id_usuario_reg
                  from wf.tobs 
                  where id_obs=v_parametros.id_obs
                  and fecha_reg::date = now()::date) = p_id_usuario ) then 
               ...
               raise exception 'La modificacion de la observacion solo la puede realizar el funcionario que registro la observacion,
               					y en la misma fecha de registro ';             
            end if;
	...
	***********************************/
		...
		begin
            ...
            if p_administrador != 1 then
                if  v_registros.fecha_reg::date != now()::date then
                    raise exception 'Solo se puede eliminar el mismo dia que se registro la observacion';
                end if;
            end if;
