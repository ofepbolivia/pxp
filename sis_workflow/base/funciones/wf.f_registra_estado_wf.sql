            ...
            -- obtener los tipos del estado padre
             ...
             IF      p_id_tipo_estado_siguiente=ANY(va_id_tipo_estado)
                 and p_id_tipo_estado_siguiente != v_registros_ant.id_tipo_estado THEN
                     ...
                     IF v_sw_error and p_id_proceso_wf not in (674538,714352,674510,674521,674482,714377,674547,674558,637171,637255,710713,632386,632379,632372,634015,653571,653391,700481,634057,634079,633980,625333,625313,625296,634005,
633991,634386,634383,634380,634378,634375,634389,623599,623588,623578,623571,623549,623534,623491,623419,623387,618897,617257,661848,682349,682354,660962,725519,725562,725568,
653545,633758,672202,677611,677626,677643,677637,653558,653564,677655,653365,677621,677650,679655,677571,677556,677512,677529,653536,653553,653346,672328,679226,653328,653526,
653521,653550,653311,701603,710145,700461,700455,700443,700474,700467,653297,677542,653380,699928,698805,698615,698623, 649038) THEN
                       raise exception 'No puede revertir revise los siguientes procesos anexos:<br>%',v_mensaje_error;
                     END IF;
    ...
    /*Se registra las alarmas que se encuentran en plantilla correo que cumplan con la regla*/
       --raise exception 'p_id_proceso_wf % p_id_tipo_estado_siguiente % p_id_estado_wf_anterior % p_id_funcionario % v_registros.plantilla_mensaje %',p_id_proceso_wf,p_id_tipo_estado_siguiente,p_id_estado_wf_anterior,p_id_funcionario,v_registros.plantilla_mensaje;
     ...
     for v_registros_correo in (select pc.*,sub.nombre as nombre_subsistema,sub.codigo as codigo_subsistema
                ...
                --raise exception 'p_id_proceso_wf % v_registros_correo.regla % p_id_tipo_estado_siguiente % v_registros_ant.id_tipo_estado %',p_id_proceso_wf,v_registros_correo.regla,p_id_tipo_estado_siguiente,v_registros_ant.id_tipo_estado;
